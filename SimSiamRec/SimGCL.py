...
class SimGCL_train_dataset(Dataset):
    def __init__(self, 
                 user_list,
                 pos_item_list,
                 neg_item_list):
...
class SimGCL(nn.Module):
    ...
    def save_model(self, model, args):
        ...
        torch.save({'model': model.state_dict()}, log_path + \
                   "%s_embed_size%d_reg%.5f_lr%0.5f_layer%.d_ssl_reg%.5f_temperature%.2f_eps%.2f_%s_%s.pth" % \
                          (args.dataset,
                           args.embedding_size, 
                           args.reg_rate, 
                           args.lr,  
...
def train(train_data, test_data, num_users, num_items):
    ...
    simgcl = SimGCL(data_train, num_users, num_items).to(device)
    ...
    train_datasets = SimGCL_train_dataset(user_list, pos_item_list, neg_item_list)
    ...
    for epoch in range(args.epochs):
        ...
        with tqdm(total=len(train_dataloader)) as t:
            for step, (user_list, pos_item_list, neg_item_list) in enumerate(train_dataloader):
                ...
                bpr_loss, ssl_loss = simgcl(u, i, j)
...
def out_max(pre3, rec3, ndcg3, pre5, rec5, ndcg5, pre10, rec10, ndcg10, pre20, rec20, ndcg20):
    ...
    if not os.path.exists(csv_path):
        with open(csv_path, 'w') as f:
            f.write("embedding_size,learning_rate,reg_rate,ssl_reg,layer,temperature,eps,noise_type,noise_type,pre3,recall3,ndcg3,pre5,recall5,ndcg5,pre10,recall10,ndcg10,pre20,recall20,ndcg20" + '\n')
            f.write(log + '\n')  
            f.close()
    else:
...
def parse_args():  
    ...
    parser.add_argument('--ssl_reg', type=float, default=2.0,
